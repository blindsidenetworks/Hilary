...
var deleteContent = module.exports.deleteContent = function(ctx, contentId, callback) {
    ...
    _canManage(ctx, contentId, function(err, contentObj) {
        ...
        ContentDAO.Content.deleteContent(contentObj, function(err, members) {
            ...
            var bbbEndPoint = bbbConfig.endpoint.indexOf('://') > -1 ? bbbConfig.endpoint.split('/')[2] : bbbConfig.endpoint.split('/')[0];
            if(contentObj.link && (contentObj.link).indexOf(bbbEndPoint) != -1) {
                var recordId = (contentObj.link).substring((contentObj.link).indexOf('meetingId=')+10);
                MeetupsAPI.Meetups.deleteRecordingLink(ctx, recordId, function(err){
                    if (err) {
                        log().info(err);
                    }
                });
